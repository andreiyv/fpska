...
rem ffmpeg -i 50_60fps.avs -c:v libx264 -preset slow -crf 15 -filter:v hqdn3d=5.0 -c:a copy 50_60fps.avi


rem %cd%/ffmpeg/ffmpeg.exe -i %cd%/scripts/50_60fps_no_GPU.avs -c:v libx264 -preset slow -crf 15 -c:a copy 50_60fps.avi



rem %cd%/x264/x264-r2762-90a61ec.exe --output "60fps.mkv" "%cd%\scripts\50_60fps_no_GPU.avs"

%cd%/x264/x264-win32.exe --preset veryslow --output "60fps.mkv" "%cd%\scripts\w8_overlap3_levels5_distance256.avs"
